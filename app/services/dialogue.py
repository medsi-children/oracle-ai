...
def format_first_contact() -> str:
    return (
        ...
        "Здесь действует Закон: каждый ответ вернется к тебе в виде будущего рейтинга. "
...
def onboarding_required_reply() -> str:
    return (
        "Сначала пройдите входную проверку.\n\n"
...
def group_only_reply() -> str:
    return (
        "Баттлы проходят в закрытом групповом чате.\n\n"
...
def format_onboarding_case(case: Case, step: int) -> str:
    return (
        f"Испытание {step}/{ONBOARDING_CASE_COUNT}\n\n"
        f"{case.prompt}\n\n"
...
def format_help() -> str:
    return (
        ...
        "Команды:\n"
        "/case — короткий кейс с оценкой и псикоинами\n"
        "/case тема — свой кейс, если куплена привилегия\n"
        "/news — реальная новость как этический кейс\n"
...
async def build_onboarding_conclusion(
    ...
    *,
    ...
    fallback = (
        "Проверка завершена, благодарим за участие в анализе!\n\n"
        ...
        "Нажимайте на кнопку снизу и следуйте указаниям. Ждём вас в системе!"
    )
    try:
            ...
            [
                {
                    "role": "system",
                    "content": (
                        "Ты Оракул. На основе семи первичных оценок дай пользователю "
                        "краткий, прямой и эстетичный вывод. Без диагнозов. Нужно: 1) что "
                        "видно по человеку, 2) зона роста, 3) финальная фраза: 'Проверка завершена, "
                        "благодарим за участие в анализе!'. Не обещай ручной контакт администратора. "
                        "В конце напиши: Нажимайте на кнопку снизу и следуйте указаниям. Ждём вас в системе! "
                        "Без повторов. Обращение на 'вы'. Не называй индекс финальной истиной и не пиши, "
                        "что человек достиг потолка: это стартовая калибровка, дальше рост идет через баттлы и обсуждения."
                    ),
                },
                {
                    "role": "user",
    ...
    if "кнопку" not in conclusion.lower():
...
async def handle_user_text(
    ...
    *,
    ...
    if session.state.startswith("onboarding:case:") and command.startswith("/") and command != "/help":
        return (
            "Вы сейчас проходите входную проверку. Завершите текущий кейс и только потом используйте другие команды.",
    ...
    if command == "/start":
        if user.lifecycle_status == "beginner" and not admin_user:
            return (
                "Проверка уже пройдена.\n\n"
                "Нажмите синюю кнопку возле поля отправки сообщений, откройте систему "
    ...
    if session.state.startswith("onboarding:case:"):
        ...
        if len(assessments) >= ONBOARDING_CASE_COUNT:
            ...
            # Add welcome message about ETHOS, subject vs object (task 3)
            welcome = (
                "\n\n**Добро пожаловать в систему!**\n\n"
                "ETHOS — это не просто игра, а цифровой аудит твоей личности. Это пространство, где искусственный интеллект (Оракул) анализирует твои решения, слова и даже «послевкусие» твоих реальных встреч, чтобы определить твой истинный вес как человека.\n"
                ...
                "Твой путь — это процесс отделения себя от навязанных программ. Оракул будет зеркалить тебя до тех пор, пока ты либо не сдашься, либо не обретешь субъектность."
            )
            conclusion = conclusion + welcome
    ...
    if command == "/buy":
        if user.lifecycle_status != "follower" and not is_admin(user):
            return (
                "Покупки откроются после входа в систему через синюю кнопку mini-app.",
                "buy_locked",
                0,
                None,
            )
    ...
    if session.state.startswith("case:"):
        ...
        reply = format_assessment_reply("Разбор", assessment, token_delta)
        reply += (
    ...
    if session.state.startswith("news:"):
        ...
        reply = format_assessment_reply("Разбор новости", assessment, token_delta)
