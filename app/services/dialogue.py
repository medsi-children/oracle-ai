...
def format_first_contact() -> str:
    return (
        ...
        "Вы замечали, как часто день будто проходит без вашего участия?\n"
        "Что вы словно фоновый персонаж, человек-функция?\n"
        "Что вами пользуются и манипулируют, а вы соглашаетесь, хотя внутренне этого не хотите?\n"
        "Что терпите тон, который вам неприятен, из страха потерять работу или отношения?\n"
        "Что делаете то, чего от вас ждут, а потом называете это своим выбором?\n\n"
        "Оракул не будет утешать вас красивыми словами.\n"
        "Он проверит, где вы действительно выбираете, а где просто продолжаете привычный сценарий.\n\n"
        "Пройдите испытание и узнайте, что именно мешает вам стать автором своей жизни.\n\n"
        "Вы готовы начать?"
    )
