...
def format_first_contact() -> str:
    return (
        "Первичный контакт <b>ETHOS</b> открыт.\n\n"
        "Оракул не ищет «правильный» образ. Он смотрит на другое: "
        "как вы выбираете под давлением, где говорите от себя, а где действуете на автомате.\n\n"
        "В системе есть два режима.\n\n"
        "<b>Объект</b> реагирует первым импульсом, подстраивается под чужой сценарий "
        "и часто называет привычку выбором.\n\n"
        "<b>Субъект</b> замечает импульс, проверяет реальность, держит границы "
        "и берет ответственность за последствия.\n\n"
        "Сейчас будет 7 коротких испытаний. Отвечайте не красиво, а точно: "
        "что вы сделаете и почему именно так.\n\n"
        "Вы готовы начать?"
    )
...
def format_onboarding_final_message() -> str:
    return (
        "<b>ETHOS</b> — это личный контур тренировки субъектности.\n\n"
        "Здесь Оракул собирает ваш профиль по решениям, словам и реакции под давлением. "
        "Цель не в том, чтобы казаться сильнее. Цель — точнее видеть свой мотив, "
        "выбирать действие и выдерживать последствия.\n\n"
        "Первичная проверка завершена. У вас уже есть стартовый индекс, первые псикоины "
        "и персональный профиль.\n\n"
        "Следующий шаг — войти в систему <b>ETHOS</b>. Внутри откроются личный кабинет, "
        "Сфера Мудрости, кейсы, баттлы и закрытое сообщество."
    )
