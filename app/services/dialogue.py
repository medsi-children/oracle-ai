...
def format_first_contact() -> str:
    return (
        "Анализ вашей личности <b>завершен</b>.\n"
        "Система зафиксировала в вас потенциал <b>субъекта</b>.\n"
        "Но потенциал — это еще не власть над собственной жизнью.\n\n"
        "Вы когда-нибудь замечали, что... \n"
        "• Ваш день проходит словно бы без вашего участия?\n"
        "• Вы словно фоновый персонаж в чьем-то чужом фильме?\n"
        "• Окружающие вами пользуются и манипулируют, а вы соглашаетесь, хотя внутренне этого не хотите?\n"
        "• Вы терпите тон и отношение, которые вам неприятны, из страха потерять работу или любовь?\n"
        "• Стараетесь быть удобным и делаете то, что от вас ожидают окружающие, а потом называете это своим выбором?\n\n"
        "Если что-то из этого для вас знакомо, значит вы <b>объект</b>.\n\n"
        "<b>ОБЪЕКТ</b> - это человек-функция. Он действует реактивно: обижается, когда его задели, верит в то, что диктует лента новостей, и использует готовые шаблоны поведения. <b>Объектом</b> легко манипулировать, потому что его реакции предсказуемы. Он — материал для чужих решений.\n\n"
        "Но вам необязательно оставаться <b>объектом</b>. Вы можете стать <b>субъектом</b>.\n\n"
        "<b>СУБЪЕКТ</b> - это человек-автор. Он обладает «внутренним стержнем» и осознанностью. <b>Субъект</b> сам выбирает свою реакцию даже в условиях давления. Он видит манипуляции, берет ответственность за свои ошибки и сохраняет верность своим ценностям, даже когда это невыгодно. Он — источник смыслов.\n\n"
...
def system_entry_reply_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
def build_probe_question(text: str, implicit: dict) -> str:
    ...
    if int(implicit.get("word_count") or 0) < 14:
        return (
            "Слишком коротко для выбора с последствиями. Какой конкретный шаг ты готов сделать, если позиция станет неудобной?"
        )
...
def format_reflection_prompt(message: str) -> str:
    return (
        "Перед фиксацией ответа есть один вопрос.\n\n"
        f"{message}\n\n"
        "Оставить так или уточнить ответ?"
    )
...
def format_help() -> str:
    return (
        ...
        "/news — новость как этический разбор\n"
        "/battle — баттл с ИИ-агентом или человеком\n"
        "/profile — профиль, индекс и баланс\n"
        "/shop — кабинет ETHOS, Сфера Мудрости и магазин\n"
        "/weekly — недельный вердикт по динамике\n\n"
        "Своя тема для /case, /news и /battle открывается после покупки привилегии. "
        "В группе доступны /joinbattle, /finishbattle и /finishdiscussion."
    )
...
def format_assessment_reply(title: str, assessment, token_delta: int) -> str:
    strongest_metric = max(
        [
            ("собственная позиция", assessment.subjectivity),
            ("честность", assessment.honesty),
            ("контроль реакции", assessment.emotional_sovereignty),
            ("проверка реальности", assessment.cognitive_humility),
            ("эмпатия", assessment.empathy),
        ],
        key=lambda item: item[1],
    )
    return (
        f"{title}\n\n"
        f"Индекс ответа: {assessment_average_score(assessment)}/100\n"
        f"Сильнее всего проявлено: {strongest_metric[0]} — {strongest_metric[1]}/100\n\n"
        "Карта ответа:\n"
        f"Позиция: {assessment.subjectivity}/100\n"
        f"Честность: {assessment.honesty}/100\n"
        f"Контроль реакции: {assessment.emotional_sovereignty}/100\n"
        f"Проверка реальности: {assessment.cognitive_humility}/100\n"
        f"Эмпатия: {assessment.empathy}/100\n\n"
        f"{assessment.summary}\n\n"
        f"Награда: {psycoins(token_delta)}"
    )
...
async def build_onboarding_conclusion(
    ...
    *,
    ...
    fallback = (
        "<b>Вывод по проверке</b>\n\n"
        "По вашим ответам виден потенциал к самостоятельному выбору, "
        "но также заметны зоны, где реакция может включаться быстрее осознанного решения. "
        "Главная точка роста — укреплять ответственность за собственную позицию "
        "и удерживать ее даже под давлением."
    )
    try:
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "1) где человек уже берет управление, 2) где действует на автомате или теряет влияние "
...
def format_onboarding_final_message() -> str:
    return (
        "<b>ETHOS</b> — это не просто игра, а цифровой аудит твоей личности. "
        "Это пространство, где искусственный интеллект (Оракул ИИ) анализирует твои решения, "
        "слова и эмоциональную реакцию, чтобы определить маршрут к становлению <b>субъектом</b>.\n\n"
        ...
        "Твой путь в <b>ETHOS</b> — это процесс отделения себя от навязанных программ. "
        "Оракул будет зеркалить тебя до тех пор, пока ты либо не сдашься, "
        "либо не обретешь <b>субъектность</b>.\n\n"
        "Ваш следующий шаг — войти в систему <b>ETHOS</b>. "
        "Станьте частью сообщества, где проходят психологические баттлы, "
        "разборы кейсов и испытания на <b>субъектность</b>. "
        "Учитесь и оттачивайте свои навыки <b>субъектности</b> и зарабатывайте "
        "внутрисистемные токены, которые впоследствии можно вывести в Телеграм Звезды."
    )


def join_onboarding_completed_reply(analysis: str, final_message: str) -> str:
    return f"{analysis.strip()}{ONBOARDING_COMPLETED_SEPARATOR}{final_message.strip()}"


def split_onboarding_completed_reply(reply: str) -> tuple[list[str], str]:
    if ONBOARDING_COMPLETED_SEPARATOR not in reply:
        return [], reply
    analysis, final_message = reply.split(ONBOARDING_COMPLETED_SEPARATOR, 1)
    extra_messages = [analysis.strip()] if analysis.strip() else []
    return extra_messages, final_message.strip()
...
async def handle_solo_game_turn(
    ...
    *,
    ...
    result_line = (
        ...
        if won
        else "Ставка не сыграла. Она списана за участие в разборе."
    )
    reply = (
        f"{title}\n\n"
    ...
    *,
    ...
    if command == "/start":
        ...
        if user.lifecycle_status == "follower" and not admin_user:
            return (
                ...
                "Откройте синюю кнопку возле поля отправки сообщений, чтобы перейти в кабинет, "
                "профиль, Сферу Мудрости и баланс.",
                "system_ready",
                0,
                None,
            )
    ...
    if session.state.startswith("onboarding:case:"):
        ...
        if len(assessments) >= ONBOARDING_CASE_COUNT:
            ...
            final_message = format_onboarding_final_message()
            user.lifecycle_status = "beginner"
            user.profile_summary = extract_profile_summary(conclusion)
            user.token_balance += 10
            return (
                join_onboarding_completed_reply(conclusion, final_message),
                "onboarding_completed",
                token_delta + 10,
                system_entry_reply_markup(),
            )
