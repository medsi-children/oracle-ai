...
def local_reflection_from_signals(text: str, implicit: dict) -> ReflectionResult:
    ...
    if implicit.get("aggression_markers"):
        return ReflectionResult(
            True,
            "В ответе есть энергия, но она уходит в атаку. Так легко проиграть управление ситуацией: вместо позиции появляется реакция.",
            "aggression",
        )
    if implicit.get("appeasement_markers"):
        return ReflectionResult(
            True,
            "Ты как будто пытаешься понравиться Оракулу. Это не усиливает позицию: субъектность начинается там, где ты готов спорить по существу.",
            "appeasement",
        )
    if float(implicit.get("cliche_density") or 0) >= 0.18:
        return ReflectionResult(
            True,
            "Ответ звучит правильно, но слишком общо. В нем не видно твоей цены выбора: что ты реально готов потерять или сделать?",
            "cliche",
        )
    if word_count < 10:
        return ReflectionResult(
            True,
            "Ответ слишком короткий для выбора с последствиями. Сейчас видна позиция, но не видно действия и цены.",
            "too_short",
        )
    ...
    if not implicit.get("uses_first_person") and word_count >= 18:
        return ReflectionResult(
            True,
            "В ответе много общего рассуждения, но мало твоего личного выбора. Система оценивает не лозунг, а управление собственной позицией.",
            "no_first_person",
        )
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "разрыв с предыдущими ответами, уход от цены выбора, смена принципа "
                        "без признания, общие слова вместо действия или сдача управления ситуации. "
