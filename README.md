...
- баттлы, кейсы и новости через выбор формата, ставку в псикоинах и подтверждение старта
- соло-режим с ИИ-агентом и групповой режим с человеком
...
- ежедневный утренний вопрос в 10:00 по московскому времени
...
- `/help` — список команд.
- `/case` — этический кейс: выбор ИИ-агента или человека, ставка, подтверждение, разбор.
- `/news` — Sentinel Mode: новость как этический кейс, ставка и подтверждение перед стартом.
- `/profile` — статус, индекс субъектности, баланс псикоинов.
- `/battle` — баттл на тему Оракула: соло с ИИ-агентом или в группе с человеком.
...
Backend запускает встроенный scheduler и каждый день в 10:00 Europe/Moscow отправляет
подходящим пользователям вопрос с точным началом:

```text
У Оракула для вас вопрос. Ответьте на него и заработаете псикоины.
```

Вопрос генерируется LLM как конкретная жизненная ситуация про управление своей реакцией,
выбором и ответственностью. В промпте запрещены слова `субъект`, `объект` и `терпила`.
Ручной запуск для проверки:

```text
POST /api/v1/daily/send-morning-case
```
